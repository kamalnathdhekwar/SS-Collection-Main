...
function Footwear() {
  ...
  const productCounts = useMemo(
    () => ({
      categories: getCounts(footwearProducts, "category"),
      brands: getCounts(footwearProducts, "brand"),
    }),
    []
  );
  ...
  const filteredProducts = useMemo(() => {
    ...
    const visibleProducts = footwearProducts.filter((product) => {
      ...
      const matchesSearch =
        query.length === 0 ||
        [product.name, product.brand, product.category, product.description, product.colors.join(" "), product.sizes.join(" ")]
          .join(" ")
          .toLowerCase()
          .includes(query);

      return matchesCategory && matchesBrand && matchesSize && matchesColor && matchesPrice && matchesRating && matchesDiscount && matchesAvailability && matchesTrending && matchesBestSeller && matchesSearch;
    });
      ...
      <section className="w-full border-b border-slate-200 bg-white">
        <div className="mx-auto w-full max-w-[1440px] px-4 py-7 sm:px-6 lg:px-8">
          <nav className="mb-4 flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-500">
            <span>Home</span>
            <span>/</span>
            <span className="text-slate-950">Footwear</span>
          </nav>
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600 sm:text-base">
                Explore running shoes, casual shoes, sneakers, sports shoes, sandals, and slippers from top footwear brands with smart filters and smooth sorting.
              </p>
            </div>
            ...
            <div className="grid w-full max-w-sm grid-cols-3 gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 text-center lg:w-auto">
              <div className="px-3">
                <p className="text-xl font-extrabold text-slate-950">{footwearProducts.length}</p>
              ...
              <div className="px-3">
                <p className="text-xl font-extrabold text-slate-950">6</p>
          ...
          <div className="min-w-0 flex-1 space-y-5">
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
                ...
                <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">Try changing the category, brand, price, size, color, rating, or search term to see more footwear.</p>
